...
> >
> > See [Operator CRD: Pools](https://silo.pigsty.io/reference/operator-crd.html#pool) for more information on all subfields.
> >
> > **servers**
> >
...
> > >
> > > This is used by MinIO to verify TLS connections from clients using those CAs If you omit this and have clients using TLS certificates minted by an external CA, those connections may fail with warnings around certificate verification. See [Operator CRD: TenantSpec](https://silo.pigsty.io/reference/operator-crd.html#tenantspec).
> >
> > **externalCertSecret**
> >
...
> > >
> > > See [Operator CRD: TenantSpec](https://silo.pigsty.io/reference/operator-crd.html#tenantspec).
> > >
> > > {{% alert color="warning" %}}
> > > **Important**
> > >
...
> > >
> > > You can pass the CA to the Operator to allow it to trust that cert. See [Self-Signed, Internal, and Private Certificates](https://silo.pigsty.io/operations/network-encryption.html#self-signed-internal-private-certificates-and-public-cas-with-intermediate-certificates) for more information. This step may also be necessary for globally trusted CAs where you must provide intermediate certificates to the Operator to help build the full chain of trust.
> > > {{% /alert %}}
> >
> > **requestAutoCert**
> >
...
> > >
> > > See [Operator CRD: CertificateConfig](https://silo.pigsty.io/reference/operator-crd.html#certificateconfig)
>
> **features**
>
> > MinIO features to enable or disable in the MinIO Tenant See [Operator CRD: Features](https://silo.pigsty.io/reference/operator-crd.html#features).
>
> **buckets**
>
...
tenant:
  ...
  # Top level key for configuring MinIO Pool(s) in this Tenant.
  #
  # See `Operator CRD: Pools <https://silo.pigsty.io/reference/operator-crd.html#pool>`__ for more information on all subfields.
  pools:
    ###
  ...
  certificate:
    ...
    # See `Operator CRD: TenantSpec <https://silo.pigsty.io/reference/operator-crd.html#tenantspec>`__.
    externalCaCertSecret: [ ]
    ###
    # Specify an array of Kubernetes secrets, where each entry corresponds to a secret contains the TLS private key and public certificate pair.
    #
    ...
    #    See `Self-Signed, Internal, and Private Certificates <https://silo.pigsty.io/operations/network-encryption.html#self-signed-internal-private-certificates-and-public-cas-with-intermediate-certificates>`__ for more information.
    ...
    #
    # See `Operator CRD: CertificateConfig <https://silo.pigsty.io/reference/operator-crd.html#certificateconfig>`__
    certConfig: { }
  ###
  # MinIO features to enable or disable in the MinIO Tenant
  # See `Operator CRD: Features <https://silo.pigsty.io/reference/operator-crd.html#features>`__.
  features:
    bucketDNS: false
    domains: { }
    enableSFTP: false
