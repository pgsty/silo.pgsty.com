...
下表描述配套服务端改动 `1a6d5b415` 之后的行为。该改动目前仅存在于本地 `pgsty/minio` 分支：尚未进入公开 `origin/master`，最新公开服务端版本 `RELEASE.2026-06-18T00-00-00Z` 也不包含它。已发布构建仍保留此前行为。在依赖这些优先级保证前，请先核对服务端发布说明。
{{% /alert %}}
