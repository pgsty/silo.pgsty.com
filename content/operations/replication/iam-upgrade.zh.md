...
该演练停在重放撤销清单或换密钥之前，**不代表可以重新开放恢复后的系统**。
实验环境为共享时钟、没有外部节点的单个隔离 Linux ARM64 容器；尚未验证生产存储快照、etcd 高可用集群、
外部身份提供方或 KMS、时钟偏差，以及回滚调和后重新开放访问。这些剩余检查及精确制品身份由
[#200](https://github.com/pgsty/silo/issues/200) 跟踪。
