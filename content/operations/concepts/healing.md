...
## When does MinIO heal an object? {#when-does-minio-heal-an-object}

MinIO has a robust system for healing objects.

### Healing during `GET` requests {#healing-during-get-requests}
...
### Healing with the object scanner {#healing-with-the-object-scanner}

MinIO uses an [object scanner](/operations/concepts/scanner/#minio-concepts-scanner) to perform a number of tasks related to objects. One of these tasks checks the integrity of objects and, if found damaged or corrupted, heals them.

On each scanning pass, MinIO uses a hash of the object name to select one out of every 1,024 objects to check.

If any object is found to have lost shards, MinIO heals the object from available shards. By default, MinIO does *not* check for [bit rot](/glossary/#term-bit-rot) corruption using the scanner. This can be an expensive operation to perform and the risk of bit rot across multiple disks is low.

### Healing by manual request {#healing-by-manual-request}

Administrators can use [`mc admin heal`](/reference/minio-mc-admin/mc-admin-heal/#command-mc.admin.heal) to initiate a full system healing. The procedure is very resource intensive and not typically needed.
