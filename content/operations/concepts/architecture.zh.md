...
**对于需要容忍节点故障的生产部署，SILO 建议以至少 4 台具有同构存储和计算资源的主机规划拓扑。这是部署建议，不是运行 SILO 或启用纠删码的最低要求。**

> 应按每个纠删集合的读写仲裁，验证规划中的节点故障是否仍可容忍。SILO 支持单节点双盘 `EC:1` 部署，但任一盘不可用时无法继续写入，主机宕机时服务也不可用。
