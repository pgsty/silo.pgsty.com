...
The `.deb` package install the following [systemd](https://www.freedesktop.org/wiki/Software/systemd/) service file to `/usr/lib/systemd/system/silo.service`:
...
MinIO expects the default certificate names of `private.key` and `public.crt` for the private and public keys respectively. Place the certificates in a directory accessible by the `silo` user/group:
