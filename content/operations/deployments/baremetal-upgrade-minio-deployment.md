...
6. **Upgrade through packages or artifacts.** SILO permanently disables in-place updates; the compatibility variable `MINIO_UPDATE` cannot re-enable them.
...
>
> Current SILO rejects in-place updates and does not download an upstream server. `MINIO_UPDATE` remains only for configuration compatibility. Replace verified packages or binaries as described below; the client command [`mc update`](/reference/minio-mc/mc-update/#command-mc.update) is also disabled.
...
2. Install the package or replace the binary on every node **without restarting only part of the cluster**:
   ...
   Verify the downloaded archive against its release checksum before extracting it. The archive checksum does not apply to the extracted executable. Then install it:

   ```shell
   sudo install -m 0755 ./silo /usr/local/bin/silo
   ```

   Replace `/usr/local/bin/silo` with the path returned by `command -v silo` when your installation uses a different location.
   {{< /tab >}}
   {{< /tabs >}}

3. Run `silo --version` on every node. Do not proceed until every node reports the same intended release.
4. Restart all server processes as one coordinated operation. Where the admin API is available, use:
   ...
   Native packages use `silo.service`; otherwise coordinate `systemctl restart silo` across all nodes through your automation. Custom installations should use their actual unit name. Do not improvise a rolling mixed-version deployment unless the target release explicitly supports it.
