...
- The MinIO Operator installed and [set up for cert-manager](/operations/cert-manager/cert-manager-operator/#minio-certmanager-operator).

## 1) Create the tenant namespace CA Issuer {#create-the-tenant-namespace-ca-issuer}
...
## 2) Create the `Issuer` {#create-the-issuer}
...
## 3) Create a certificate for the tenant {#create-a-certificate-for-the-tenant}
...
## 5) Trust the tenant’s CA in MinIO Operator {#trust-the-tenant-s-ca-in-minio-operator}
...
### Create `operator-ca-tls-tenant-1` secret {#create-operator-ca-tls-tenant-1-secret}
...
With the Certificate Authority and `Issuer` in place for the tenant’s namespace, you can now [deploy the object store tenant](/operations/deployments/k8s-deploy-minio-tenant-on-kubernetes/#minio-k8s-deploy-minio-tenant).
