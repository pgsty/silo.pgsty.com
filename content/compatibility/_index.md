---
title: "Compatibility"
linkTitle: "Compatibility"
description: "Where Silo matches MinIO and where it deliberately differs — server, client, and console."
url: "/compatibility/"
weight: 7
type: docs
icon: fa-solid fa-code-compare
minio_origin: false
silo_modified: false
---

Silo is a community fork of MinIO, so the two remain close relatives rather than separate products. This section records, component by component, what Silo keeps from MinIO, where it deliberately differs, and what either means if you are moving between them.

One page per component: the `silo` server, the `mcli` client, and the web Console.
