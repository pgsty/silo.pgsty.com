...
> **先声明证据类别。** 以下每个机制都经合成实验验证：ClientHello 抓取、新鲜进程 CA 探针、夹具复现。#154 客户的 discovery URL 与入口配置始终未获得，因此**不对该部署做任何根因断言**——两个本地已验证的机制都能产生所报症状：入口拒绝新握手，或代理拒绝变更后的 User-Agent，均有可能。#154 保持打开，等待受影响环境复测。
