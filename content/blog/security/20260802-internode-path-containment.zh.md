...
**状态：** 已修复，**未披露**（未申请 CVE/GHSA，上游仓库已归档）
...
- **`ReadFileHandler` 无上界分配**——合法值可达数百 MiB，**不存在小而安全的上界**；干净修法需先 stat 再 clamp，要独立的调用方审计。不在安全补丁里凭猜测引入可能拒绝合法请求的上界。
- **元数据写入路径未校验 part size**——恶意 peer 可写入毒化 `xl.meta`，之后本地修复静默跳过。完整规则需先确认合法对象的 `Erasure.BlockSize` 恒大于零，而 `FileInfo.IsValid()` **不校验它**。
- **Windows 无 CI**——发布 Windows 构建，测试只跑 Ubuntu。Windows 规则是从文档化的 Win32 行为推理而来，未经实机验证。
- **符号链接**——词法防御，与上游一致。

## 尾声 {#closing}

上一篇说"关闭一个 endpoint，与关闭一个缺陷类别，是两种不同结论"。这次我们做完了那个类别的审计，代价是四次自制回归和三次被推翻的"可以发版"。
