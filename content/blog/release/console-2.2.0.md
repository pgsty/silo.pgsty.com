...
date: 2026-08-25
author: "Ruohang Feng"
description: "Draft notes for SILO Console 2.2.0: a text preview for logs and structured text, a download path that cannot lie, repaired permission gating, an untangled user API, hardened notification forms, and the dependency stack re-platformed on Go 1.27 and the maintained SILO forks."
...
> [!CAUTION]
> **Draft — not released.** The 2.2.0 candidate is code-complete at commit [`19047161f`](https://github.com/pgsty/silo-console/commit/19047161fa0e6ad6436057e5cc996ac6eb0751e4) and its verification matrix is green, but there is no release tag, binary, package, container image, checksum, or signature yet. Scope and date may still change before publication.

**Candidate version:** `v2.2.0` · **Candidate commit:** `19047161f` · **Status:** draft, pending release decision · **Repository:** [pgsty/silo-console](https://github.com/pgsty/silo-console)
...
Underneath, the dependency stack moves to Go 1.27.0 and onto the maintained SILO forks: [`pgsty/silo-pkg` 3.12.0](/blog/release/pkg-3.12.0/) (published 2026-08-24) replaces upstream `minio/pkg`, `pgsty/mc` replaces upstream `mc` as a library, and the etcd client line moves to 3.7.1, closing [CVE-2026-73500](https://pkg.go.dev/vuln/GO-2026-6107).

The change set since [v2.1.1](https://github.com/pgsty/silo-console/releases/tag/v2.1.1) is 24 commits touching 161 source files (+7,649/−1,930 lines, not counting the regenerated embedded frontend assets).
...
- **`silo-pkg` 3.12.0 is real now.** The earlier draft of these notes described a staging arrangement; [`pgsty/silo-pkg` v3.12.0](/blog/release/pkg-3.12.0/) was published on 2026-08-24 and the console pins it directly. It carries the policy resource-boundary hardening, condition-key lookup repair, LDAP and certificate-watcher fixes reviewed in [3.11.0](/blog/release/pkg-3.11.0/), plus the Go 1.27 / etcd 3.7 baseline.
- **The `mc` library moved to the `pgsty/mc` fork** (date-tagged pseudo-version), which the earlier draft had deferred. Import paths are unchanged.
- **The `require` line stays on upstream `v3.6.1` deliberately.** Go ignores `replace` directives in dependency modules, so a downstream module that requires this console resolves *upstream* `minio/pkg` — and `v3.12.0` does not exist as an upstream tag. Requiring a real upstream tag keeps the console resolvable downstream; the replace applies the fork for the console's own builds. This was verified empirically: with **every replace stripped**, `go mod tidy` selects upstream v3.6.1/mc/go-systemd and `go build ./...` succeeds. Note this is compile-compatibility — a downstream build without its own replace gets upstream *behavior*, not the fork's SILO-specific IAM semantics.
...
Changes required by the accepted first-party moves: `jwx` v2→v3 with `httprc` v3 (the maintained JWT line, required by silo-pkg), `go-openapi/swag/conv`+`typeutils` 0.28.0, `grpc-gateway` 2.29.0 (declared by etcd 3.7.1), `cheggaaa/pb` 1.0.30, `go.yaml.in/yaml/v3` 3.0.5. Deliberately **not** taken, after source-diff review rather than version-number trust: `minio-go` 7.0.100 (19 files across signing/endpoint/cache paths), the newer go-openapi patch lines (17–52 files each, SSRF-sensitive expander/loader paths in `spec`), `pb/v3` 3.2, `klauspost/compress` 1.19, `testify` 1.12. No forcing CVE reaches any of them.
...
- `govulncheck` at the candidate tree: **zero vulnerable symbols reached, zero vulnerable imported packages** (re-verified during this review, not carried over from the earlier draft). The Swagger build tool scans clean separately.
...
Neither is a 2.2.0 regression — both predicates were entirely dead in 2.1.1 — and in both cases the server still enforces the real rules; the recommendation is to fix both predicates before or shortly after tagging. Two sharp edges are recorded as known limitations rather than defects: the new PostgreSQL DSN parser accepts only canonical `key=value` syntax when populating structured fields (an unusual-but-libpq-valid DSN shows empty structured fields, and editing a structured field then rebuilds the DSN from those fields), and TestCafe/Playwright coverage asserts UI gating while live-server deny-path coverage remains the integration suites' job.

## Verification {#verification}

All of the following ran against the final candidate tree (`19047161f`):

**Local gates** — `go build ./...`, `go vet ./...` (plus `-tags testrunmain`), `gofmt`, `golangci-lint` (0 issues), `go test -race ./...` (api, api/policy, cmd, pkg suites), `go tool swagger validate`, `govulncheck` (0 reachable), TypeScript `tsc`, Playwright unit project (**77/77 passed**), Prettier, knip, production cross-compiles for linux/amd64 and linux/arm64 with the release tags, `go mod verify`.

**Embedded-assets determinism** — the frontend was rebuilt from source through the full pipeline (`yarn build` + embed optimization) and the result compared against the committed `web-app/build`: **byte-identical, zero dirty files**. The binary a release build embeds is exactly what the source produces.

**CI, full matrix** — GitHub Actions run [32824984359](https://github.com/pgsty/silo-console/actions/runs/32824984359) (manual dispatch, 2026-08-25, at exactly this commit): all ~30 jobs green — lint, semgrep, unit tests, five cross-compile targets, Swagger-drift check, the Docker-backed **integration** suite (distributed MinIO), **site replication**, the hermetic **SSO** suite against pinned `pgsty/silo:RELEASE.2026-08-06T00-00-00Z`, the complete **TestCafe permissions matrix** (suites 1–8, A, B), subpath-nginx, Playwright (89 passed), and the coverage gate. The Vulnerability Check workflow passed the same day. This is the first CI baseline for the fork — note that it runs **only via manual `workflow_dispatch`**; push triggers do not fire on this fork.
...
This draft is not evidence that v2.2.0 exists. Before flipping `draft: false`:

1. decide on the two inert row-guard fixes ([above](#regression-review)) — if taken, rerun the frontend gates, refresh the embedded assets, and re-dispatch CI;
2. make the release decision, tag the verified tree, and run the goreleaser pipeline (by `workflow_dispatch`, as with every previous release);
3. verify every public artifact — binaries, packages, container images, checksums, signatures — against the tagged tree;
4. update this page with the final tag, date, and artifact links.
...
- [silo-pkg 3.12.0 release notes](/blog/release/pkg-3.12.0/) · [silo-pkg 3.11.0 release notes](/blog/release/pkg-3.11.0/)
