...
**版本：** `RELEASE.2026-08-06T00-00-00Z` · **提交：** `062a91beeded62261c218db7e7120cd17e247c89`
...
本版本覆盖 `RELEASE.2026-08-04T00-00-00Z` 之后的 **24 个提交**，变更 **394 个文件**，**新增 26,864 行、删除 19,537 行**。发布前通过了六阶段验收，包括一次真实的四节点 TLS 集群从 MinIO 到 Silo 的迁移——数据逐字节校验、维护闸门滚动重启、故障注入，以及完整的回滚彩排。
...
RPM、DEB、APK 软件包恰好安装六个文件：`/usr/bin/silo`、`silo.service`、sysusers 定义（创建 `silo` 系统用户）、`/etc/default/silo`（config/noreplace）、LICENSE 与 NOTICE。RPM 以 PGSTY 维护者密钥 GPG 签名（`9592A7BC 7A682E73 33376E09 E7935D8D B9BD8B20`）。
