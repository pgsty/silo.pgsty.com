...
[v2.1.1](https://github.com/pgsty/silo-console/releases/tag/v2.1.1) 与本版本同日发布，补全了下文所述的图例加固：图例构造器无法解析的标签占位符会被直接移除，而不是把字面花括号漏进 Traffic 图表的图例；最后一处替换分支也完成了转义加固，标签值中的 `$&` 或 `$1` 不会再被当作替换指令；License 页面显示的版本号也从 2.0.0 修正为实际版本。除此之外没有任何变化——直接升级到 2.1.1 即可，本文所有内容照旧适用。
{{% /alert %}}
